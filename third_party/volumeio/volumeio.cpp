...
// renamed ddsbase.cpp -> volumeio.cpp @Adrian Derstroff
// disabled warnings @Gabriel Borrelli
...
inline void DDS_swapuint(unsigned int* x)
{
    ...
    *x = ((tmp & 0xff) << 24) | ((tmp & 0xff00) << 8) | ((tmp & 0xff0000) >> 8) | ((tmp & 0xff000000) >> 24);
}
    ...
    unsigned int block = 0)
{
    ...
    while (cnt++ < bytes) {
        ...
        if (cnt1 + cnt2 < (1 << DDS_RL) && (cnt1 + cnt2) * intmax(bits1, bits2) < cnt1 * bits1 + cnt2 * bits2 + DDS_RL + 3) {
            cnt2 += cnt1;
            if (bits1 > bits2)
                bits2 = bits1;
        } else {
    ...
    unsigned char* comment)
{
    ...
    if (description == NULL && courtesy == NULL && parameter == NULL && comment == NULL) {
        ...
        if (description != NULL)
            len1 = strlen((char*)description) + 1;
        if (courtesy != NULL)
            len2 = strlen((char*)courtesy) + 1;
        if (parameter != NULL)
            len3 = strlen((char*)parameter) + 1;
        if (comment != NULL)
            len4 = strlen((char*)comment) + 1;
...
void convfloat(unsigned char* data, unsigned int bytes)
{
    ...
    for (ptr = data, i = 0; i < bytes / 4; i++, ptr += 4) {
        ...
        data[2 * i] = ((int)floor(65535.0f * v + 0.5f)) / 256;
        data[2 * i + 1] = ((int)floor(65535.0f * v + 0.5f)) % 256;
    }
}
    ...
    unsigned int i, unsigned int j, unsigned int k)
{
    ...
    if (i > 0)
        if (i < width - 1)
            gx = (DDS_get(data, width, height, depth, i + 1, j, k) - DDS_get(data, width, height, depth, i - 1, j, k)) / 2.0;
        else
            gx = DDS_get(data, width, height, depth, i, j, k) - DDS_get(data, width, height, depth, i - 1, j, k);
    else if (i < width - 1)
        gx = DDS_get(data, width, height, depth, i + 1, j, k) - DDS_get(data, width, height, depth, i, j, k);
    else
        gx = 0.0;

    if (j > 0)
        if (j < height - 1)
            gy = (DDS_get(data, width, height, depth, i, j + 1, k) - DDS_get(data, width, height, depth, i, j - 1, k)) / 2.0;
        else
            gy = DDS_get(data, width, height, depth, i, j, k) - DDS_get(data, width, height, depth, i, j - 1, k);
    else if (j < height - 1)
        gy = DDS_get(data, width, height, depth, i, j + 1, k) - DDS_get(data, width, height, depth, i, j, k);
    else
        gy = 0.0;

    if (k > 0)
        if (k < depth - 1)
            gz = (DDS_get(data, width, height, depth, i, j, k + 1) - DDS_get(data, width, height, depth, i, j, k - 1)) / 2.0;
        else
            gz = DDS_get(data, width, height, depth, i, j, k) - DDS_get(data, width, height, depth, i, j, k - 1);
    else if (k < depth - 1)
        gz = DDS_get(data, width, height, depth, i, j, k + 1) - DDS_get(data, width, height, depth, i, j, k);
    else
        gz = 0.0;
    ...
    BOOLINT linear, BOOLINT nofree)
{
    ...
    if ((data3 = (unsigned short int*)malloc(width * height * depth * sizeof(unsigned short int))) == NULL)
        ERRORMSG();
    ...
    if (linear)
        for (i = 0; i < 65536; i++)
            err[i] = 255 * (double)(i - vmin) / (vmax - vmin);
    else {
        for (i = 0; i < 65536; i++)
            err[i] = 0.0;
        ...
        if (err[65535] > 0.0f)
            for (i = 0; i < 65536; i++)
                err[i] *= 255.0 / err[65535];
    }

    if ((data2 = (unsigned char*)malloc(width * height * depth)) == NULL)
        ERRORMSG();
